...
export default function Home() {
  ...
  const filtered = products.filter((p) => {
    ...
    return matchCat && productName.toLowerCase().includes(search.toLowerCase());
  });
    ...
    <div className="bg-gray-50 dark:bg-zinc-950 min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pt-8">
        ...
        <div className="relative mb-5 max-w-md">
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-gray-900 dark:text-white placeholder-gray-400 outline-none focus:ring-2 focus:ring-violet-400 text-sm transition-all"
          />
        ...
        <div className="flex gap-2 flex-wrap mb-6">
          {cats.map((cat, i) => (
            <button key={cat} onClick={() => setActiveCatIndex(i)}
              className={`px-4 py-1.5 rounded-xl text-xs font-semibold cursor-pointer border transition-all
                ${activeCatIndex === i
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-12">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">{t("products.title")}</h2>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-2xl w-full max-w-lg overflow-hidden">
        <div className="relative h-64 bg-gray-100 dark:bg-zinc-800">
          ...
          <button onClick={onClose}
            className="absolute top-4 right-4 w-9 h-9 rounded-xl bg-white/90 dark:bg-zinc-800/90 backdrop-blur-sm flex items-center justify-center hover:scale-110 cursor-pointer border-none shadow-sm">
            <FaTimes size={14} className="text-gray-600 dark:text-gray-300" />
          </button>
          <button onClick={() => toggleLike(product)}
            className="absolute top-4 right-16 w-9 h-9 rounded-xl bg-white/90 dark:bg-zinc-800/90 backdrop-blur-sm flex items-center justify-center hover:scale-110 cursor-pointer border-none shadow-sm">
          ...
          <div className="flex items-center gap-2 mb-4">
            {[...Array(5)].map((_, i) => (
              <FaStar key={i} size={13}
                className={i < Math.floor(product.rating) ? "text-yellow-400" : "text-gray-200 dark:text-zinc-700"} />
            ))}
