  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-zinc-950 transition-colors duration-300">
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300
        ${scrolled ? "bg-white/90 dark:bg-zinc-950/90 backdrop-blur-xl shadow-sm border-b border-gray-100 dark:border-zinc-800" : "bg-transparent"}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          ...
          <nav className="hidden md:flex items-center gap-7">
            <NavLink to="/about" className={navLinkClass}>{t("about") || "О нас"}</NavLink>
            <NavLink to="/contact" className={navLinkClass}>{t("contactText") || "Контакты"}</NavLink>
          </nav>
          ...
          <div className="flex items-center gap-2">
            ...
            <NavLink to="/likes"
              className={({ isActive }) =>
                ...
                ${isActive ? "bg-rose-50 dark:bg-rose-900/30 border-rose-300 dark:border-rose-700 text-rose-500" : "border-gray-200 dark:border-zinc-700 bg-gray-100 dark:bg-zinc-800 text-gray-500 dark:text-gray-400 hover:text-rose-500"}`}>
              <FaRegHeart size={15} />
            </NavLink>
            ...
            <NavLink to="/buys"
              className={({ isActive }) =>
                `w-9 h-9 rounded-xl flex items-center justify-center border transition-all duration-200 hover:scale-105 no-underline
                ${isActive ? "bg-violet-50 dark:bg-violet-900/30 border-violet-300 dark:border-violet-700 text-violet-500" : "border-gray-200 dark:border-zinc-700 bg-gray-100 dark:bg-zinc-800 text-gray-500 dark:text-gray-400 hover:text-violet-500"}`}>
                  ...
                  <div className="absolute right-0 top-12 w-52 bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-2xl shadow-xl py-2 z-50">
                    ...
                    {user.role === "admin" && (
                      <button onClick={() => { navigate("/admin"); setUserMenuOpen(false); }}
                        className="w-full px-4 py-2 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors cursor-pointer bg-transparent border-none">
                        🔧 Панель админа
                      </button>
                    )}
                    {user.role === "seller" && (
                    ...
                    <button onClick={handleLogout}
                      className="w-full px-4 py-2 text-left text-sm text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 transition-colors cursor-pointer bg-transparent border-none flex items-center gap-2">
                      <FaSignOutAlt size={12} /> Выйти
                    </button>
                  </div>
                )}
              </div>
        ...
        {menuOpen && (
          <div className="md:hidden bg-white dark:bg-zinc-900 border-t border-gray-100 dark:border-zinc-800 px-4 py-4 flex flex-col gap-2">
            {[{ to: "/about", label: "О нас" }, { to: "/contact", label: "Контакты" }].map(({ to, label }) => (
            ...
            {!user ? (
              <>
                <NavLink to="/login" onClick={() => setMenuOpen(false)} className="text-sm font-medium py-2 px-3 rounded-lg no-underline text-gray-700 dark:text-gray-300">Войти</NavLink>
                <NavLink to="/register" onClick={() => setMenuOpen(false)} className="text-sm font-semibold py-2 px-3 rounded-lg no-underline bg-violet-600 text-white text-center">Регистрация</NavLink>
              </>
            ) : (
      ...
      <footer className="border-t border-gray-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 mt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 grid grid-cols-1 sm:grid-cols-3 gap-8">
          <div>
            ...
            <p className="text-gray-400 dark:text-zinc-500 text-sm leading-relaxed">Современный маркетплейс для покупки и продажи товаров онлайн.</p>
          </div>
          <div>
            ...
            {[["О нас", "/about"], ["Контакты", "/contact"], ["Избранное", "/likes"], ["Корзина", "/buys"]].map(([label, path]) => (
              <div key={path} className="mb-2">
                <NavLink to={path} className="text-gray-400 dark:text-zinc-500 hover:text-violet-500 text-sm no-underline transition-colors">{label}</NavLink>
              </div>
            ))}
          </div>
          <div>
            ...
            {!user
              ...
              : <p className="text-sm text-gray-400">Вы вошли как <span className="text-violet-500 font-medium">{user.name}</span></p>
            }
          </div>
        </div>
